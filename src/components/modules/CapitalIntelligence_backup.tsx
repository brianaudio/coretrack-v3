...
export default function CapitalIntelligence() {
    ...
    const loadCapitalData = async () => {
      ...
      try {
        ...
        const recentSales = recentOrders.filter(order => {
          // Fix: Consistent Firebase timestamp handling
          try {
            const orderDate = order.createdAt?.toDate?.() || 
                            (order.createdAt ? new Date(order.createdAt) : new Date())
            return orderDate >= thirtyDaysAgo
          } catch (error) {
            console.warn('Invalid date format for order:', order.id, error)
            return false
          }
        })
        
        const totalRecentSales = recentSales.reduce((sum, order) => sum + (order.total || 0), 0)
        // Fix: Proper daily average calculation to prevent division issues
        const daysPassed = Math.max(1, Math.min(30, Math.ceil((new Date().getTime() - thirtyDaysAgo.getTime()) / (1000 * 60 * 60 * 24))))
        const avgDailySales = totalRecentSales / daysPassed
        ...
        // Fix: Safe capital recovery time calculation with proper division protection
        const capitalRecoveryTime = avgDailySales > 0 && totalInventoryValue > 0 
          ? totalInventoryValue / avgDailySales 
          : totalInventoryValue > 0 ? Infinity : 0        // Generate recommendations based on real data
        ...
        if (capitalRecoveryTime !== null && capitalRecoveryTime > 60) {
          recommendations.push({
        ...
        const calculatedMetrics = {
          ...
          capitalRecoveryTime: isFinite(capitalRecoveryTime) ? Math.round(capitalRecoveryTime * 10) / 10 : 999,
          ...
          recentPurchases: purchases.slice(0, 5).map((p, index) => {
            ...
            return {
              ...
              // Fix: Consistent date handling for purchase orders
              date: (() => {
                try {
                  return p.createdAt?.toDate?.() || 
                         (p.createdAt ? new Date(p.createdAt) : new Date())
                } catch (error) {
                  console.warn('Invalid date format for purchase:', p.id, error)
                  return new Date()
                }
              })()
            }
        ...
        // Fix: Better error handling with user feedback
        setCapitalData(prev => ({
          ...prev,
          moneyFlowAnalysis: {
            type: 'error',
            ...
            isGood: false,
            details: ['Data loading failed', 'Please refresh the page']
          }
        }))
      } finally {
        setLoading(false)
  ...
  const getEfficiencyScore = (icr: number, recoveryTime: number, velocity: number) => {
    // Fix: Handle infinity and invalid values properly
  ...
  const getStockHealthStatus = () => {
    ...
    // Fix: Handle invalid days values
