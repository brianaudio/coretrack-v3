...
export default function ShiftStatusBar() {
  ...
  const checkIfQuickCheckNeeded = async (): Promise<boolean> => {
    // Smart logic: Check if high-risk conditions detected
    const now = new Date();
    const shiftDuration = activeShift ? now.getTime() - new Date(activeShift.startTime).getTime() : 0;
    const hoursWorked = shiftDuration / (1000 * 60 * 60);
    
    // Recommend check for shifts longer than 4 hours or evening/overnight shifts
    return hoursWorked > 4 || ['evening', 'overnight'].includes(activeShift?.shiftType || '');
  };
  ...
  const getTodaysQuickCheckItems = (): QuickCheckItem[] => {
    ...
    // Smart selection based on shift type and time
    if (activeShift?.shiftType === 'evening' || activeShift?.shiftType === 'overnight') {
      // Add high-theft-risk items for evening/night shifts
      baseItems.push({ id: 'cooking_oil', name: '🛢️ Cooking Oil', expectedCount: 8, costPerUnit: 180 });
    }

    return baseItems.slice(0, 3); // Always keep it to 3 items max
  };
