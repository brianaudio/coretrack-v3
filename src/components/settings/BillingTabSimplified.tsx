...
export default function BillingTab() {
  ...
  const handleDirectPayPalSubscription = async (planId: string, paypalPlanId: string) => {
    ...
    setLoading(true)
    try {
      // Create PayPal subscription directly
      const response = await fetch('/api/paypal/create-subscription', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          planId: paypalPlanId,
          returnUrl: `${window.location.origin}/dashboard/settings?tab=billing&success=true`,
          cancelUrl: `${window.location.origin}/dashboard/settings?tab=billing&cancelled=true`
        })
      })
  ...
  useEffect(() => {
    if (subscription && isActive) return // Skip if already subscribed
    ...
    const initializePayPalButtons = async () => {
      try {
        ...
        // Initialize button for each plan
